...
def camera():
    ...
    while True:
        ...
        for top, right, bottom, left in faceCurFrame:
            ...
            cv2.putText(img, name, (left * 4 + 4, bottom * 4 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
####################################^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^Camera^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
def logout():
    file_path = "Main/Loginpage copy.py"
    root.destroy()
...
Home_indicate=Button(frame,width=27,pady=3,text='Camera', bg='#56a1f8',fg='white',border=1).place(x=2,y=3)
...
Home_indicate=Button(frame,width=27,pady=3,text='Logout', bg='#56a1f8',fg='white',border=1,command=logout).place(x=597,y=3)
